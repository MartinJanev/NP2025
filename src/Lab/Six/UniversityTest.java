...
class University {
    ...
    public University mergeFourSmallestDepartments() {
        ...
        Department mergedDepartment = smallestDepartments.stream()
                .reduce((d1, d2) -> new Department(
                        d1.getName() + " & " + d2.getName(),
                        Stream.concat(d1.getCourses().stream(), d2.getCourses().stream())
                                .collect(Collectors.toList())
                ))
                .orElseThrow(() -> new IllegalStateException("Not enough departments to merge"));

        List<Department> updatedDepartments = new ArrayList<>(departments);
        updatedDepartments.removeAll(smallestDepartments);
