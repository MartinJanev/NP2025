...
class LibrarySystem {
    ...
    public void printMembers() {
        membersById.values().stream()
                .sorted(
                        comparingInt(Member::getCurrentlyBorrowedCount).reversed()
    ...
    public void printBooks() {
        booksByIsbn.values().stream()
                .sorted(
                        comparingInt(Book::getTotalBorrows).reversed()
    ...
    public Map<Boolean, List<Member>> getMembersByBorrowActivity() {
        return membersById.values().stream()
                .collect(Collectors.groupingBy(
                        m -> m.getTotalBorrows() > 0,
                        Collectors.toList()
                ));
    }

    public Map<Integer, Set<Member>> getMembersGroupedByBorrowedCount() {
        return membersById.values().stream()
                .collect(Collectors.groupingBy(
                        Member::getCurrentlyBorrowedCount,
                        () -> new TreeMap<>(Comparator.reverseOrder()),
                        Collectors.toSet()
                ));
    }

//    public Map<String, Book> getTopBookByAuthor() {
//        List<String> authors = booksByIsbn.values().stream()
//                .map(Book::getAuthor)
//                .distinct()
//                .collect(Collectors.toList());
//
//        Map<String, Book> topBooks = new HashMap<>();
//        authors.forEach(author -> topBooks.putIfAbsent(author,
//                booksByIsbn.values().stream()
//                        .filter(book -> book.getAuthor().equals(author))
//                        .max(comparingInt(Book::getTotalBorrows)
//                                .thenComparingInt(Book::getYear))
//                        .orElse(null)
//        ));
//        return topBooks;
...
//        List<String> isbns = booksByIsbn.values().stream()
//                .map(Book::getISBN)
//                .distinct()
//                .collect(Collectors.toList());
//        Map<String, Integer> waitingListSizes = new TreeMap<>();
//
//        isbns.forEach(isbn -> waitingListSizes.put(isbn, booksByIsbn.get(isbn).getWaitingList().size()));
//
//        return waitingListSizes.entrySet().stream().sorted(
//                        Comparator.<Map.Entry<String, Integer>>comparingInt(Map.Entry::getValue)
//                                .reversed()
//                                .thenComparing(Map.Entry::getKey)
//                )
