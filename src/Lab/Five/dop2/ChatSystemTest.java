...
class ChatSystem {
    ...
    public Map<String, Set<String>> getAllRoomsByUsers() {
        ...
        registeredUsers
                .forEach(user -> {
                    Set<String> userRooms = new TreeSet<>();
                    rooms.values().stream()
                            .filter(room -> room.hasUser(user))
                            .forEach(room -> userRooms.add(room.getName()));
                    result.put(user, userRooms);
                });
        return result;
    }

    public Map<ChatRoom, Integer> getChatRoomStatistics() {
        Map<ChatRoom, Integer> stats = new TreeMap<>();
        List<ChatRoom> sortedRooms = new ArrayList<>(rooms.values());
        Collections.sort(sortedRooms);
        sortedRooms.forEach(room -> stats.put(room, room.getUsers().size()));
        return stats;
    }
