...
class GameRoom {
    ...
    // Посебен thread (single-thread executor) што FIFO ги процесира акциите
    private void startProcessor() {
        executor.submit(() -> {
            try {
                while (running || !actionQueue.isEmpty()) {
                    // poll со timeout за да не блокира бесконечно
    ...
    public void shutdown() {
        // 1) кажи му на processor loop-от да заврши кога ќе ја исцеди редицата
        running = false;

        // 2) graceful shutdown на executor
        executor.shutdown();
        try {
...
class GameServer {
    ...
    // Dispatcher thread: чита од inputQueue, наоѓа/креира соба, проследува акција
    ...
    public void shutdown() {
        // 1) запре dispatcher loop-от
        running = false;

        // 2) graceful shutdown на dispatcher executor
        dispatcher.shutdown();
        try {
        ...
        // 3) исклучи ги сите соби (тие ќе си ги исцедат редиците)
