...
class Ad implements Comparable<Ad> {
    ...
    public int compareTo(Ad o) {
        Comparator<Ad> comparator = Comparator.comparingDouble(Ad::getBidValue).reversed().thenComparing(Ad::getId);
...
class AdRequest {
    ...
    public int keywordsCount() {
        return (int) Arrays.stream(keywords.trim().split("\\s+")).count();
    }
...
class AdNetwork {
    ...
    public void readAds(BufferedReader in) throws IOException {
        ...
        while ((line = in.readLine()) != null && !line.trim().isEmpty()) {
            ...
            String content = String.join(" ", Arrays.copyOfRange(words, 4, words.length));
    ...
    public void placeAds(BufferedReader in, int k, PrintWriter out) throws IOException {
        List<AdRequest> requests = new ArrayList<>();
        String line;
        //SKip the empty line between ads and requests
        ...
        requests = requests.stream()
                .sorted(Comparator.comparingInt(AdRequest::keywordsCount).reversed()
                        .thenComparingDouble(r -> Math.abs(r.getFloorBid() - 1.0))
                        .thenComparing(AdRequest::getId))
                .collect(Collectors.toList());

        for (AdRequest req : requests) {
            List<Ad> filteredAds = ads.stream().filter(ad -> ad.getBidValue() >= req.getFloorBid()).collect(Collectors.toList());
            ...
            List<Ad> sortedAds = filteredAds.stream()
                    .sorted(Comparator.comparingInt(Ad::getImpressions)
                            .thenComparing(Comparator.comparingDouble(Ad::getTotalScore).reversed()))
                    .limit(k)
                    .sorted()
                    .collect(Collectors.toList());
            //.sorted(Comparator.comparingDouble(Ad::getTotalScore).reversed()).limit(k).sorted() // користи compareTo во Ad
            //.collect(Collectors.toList());
            // 4. печатење
