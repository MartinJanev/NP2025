...
class University {
    ...
    public University mergeFourSmallestDepartments() {
        ...
        List<Department> smallestDepartments = departments.stream()
        ...
        Department merged = smallestDepartments.stream()
                .reduce((d1, d2) ->
                        new Department(
                                d1.getName() + " & " + d2.getName(),
                                Stream.concat(
                                        d1.getCourses().stream(), d2.getCourses().stream()
                                ).collect(Collectors.toList())
                        ))
                .orElseThrow(() -> new IllegalStateException("ABCD"));

        List<Department> updated = new ArrayList<>(departments);
        updated.removeAll(smallestDepartments);
        updated.add(merged);
