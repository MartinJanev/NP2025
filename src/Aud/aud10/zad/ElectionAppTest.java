...
class InvalidVotesException extends Throwable {
    public InvalidVotesException(String s) {
    }
}
...
class VotersTurnoutApp implements Subscriber {
    ...
    Map<Integer, Integer> castedVotesPerUnitMap = new TreeMap<>();
    ...
    public void updateVotes(int unit, String pollId, String party, int votes, int totalVotersPerPoll, int totalVotersPerUnit) {
        ...
        castedVotesPerUnitMap.put(unit, 0);
        castedVotesPerUnitMap.put(unit, castedVotesPerUnitMap.get(unit) + votes);
    }
    ...
    public void printStatistics() {
        ...
        for (int unit = 1; unit <= 6; unit++) {
            if (castedVotesPerUnitMap.containsKey(unit)) {
                System.out.printf(
                        "%5d %7d %7d %7.2f%%%n",
                        unit,
                        castedVotesPerUnitMap.get(unit),
                        totalVotersPerUnitMap.get(unit),
                        100.0 * castedVotesPerUnitMap.get(unit) / totalVotersPerUnitMap.get(unit)
                );
            }
        }
...
class VotesController {
    ...
    void addVotes(String pollId, String party, int votes) throws InvalidVotesException {
        if (!parties.contains(party)) {
            throw new InvalidVotesException("Invalid party: " + party);
        }

        this.units.get(unitPerPoll.get(pollId)).addVotes(pollId, party, votes);
    }
}
