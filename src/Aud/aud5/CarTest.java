...
//                    .collect(Collectors.toList());
    ...
    public Map<String, List<Car>> groupByManufacturer() {
        ...
        for (Car car : cars) {
            ...
            map.computeIfAbsent(manufacturer, k -> new ArrayList<>()).add(car);
        }
        return map;

    }

    public Map<String, List<Car>> groupByModel() {
        Map<String, List<Car>> map = new HashMap<>();
        for (Car car : cars) {
            map.computeIfAbsent(car.getModel(), k -> new ArrayList<>()).add(car);
        }
        return map;
    }
...
public class CarTest {
    public static void main(String[] args) {
        ...
        print(result);
        System.out.println("=== Grouped By Model ===");
        Map<String, List<Car>> map = carCollection.groupByModel();
        printMap(map);
        System.out.println("=== Grouped By Manufacturer ===");
        Map<String, List<Car>> map1 = carCollection.groupByManufacturer();
        printMap(map1);

    }

    static void printMap(Map<String, List<Car>> map) {
