...
class FileSystem {
    ...
    public void addFile(char folder, String name, int size, LocalDateTime createdAt) {
        Set<File> filesIn = files.computeIfAbsent(folder, file -> new TreeSet<>(FILE_COMPARATOR));
        filesIn.add(new File(name, size, createdAt));
    }
    ...
    public int totalSizeOfFilesFromFolders(List<Character> folders) {
        return folders.stream()
                .mapToInt(f -> files.get(f).stream().mapToInt(File::getSize).sum())
                .sum();
    }
